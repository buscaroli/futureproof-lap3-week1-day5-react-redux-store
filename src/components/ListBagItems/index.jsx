...
function ListBagItems() {
  ...
  const renderItems = items.length > 0 ? items.map(item => <ItemLineCheckout key={item.id} id={item.id} title={item.title} price={item.price}/>) : <li>Bag is Empty</li>

  return (
    <ul className={styles.wrapper}>
      {renderItems}
      <ItemLineCheckout key={0} title={'TOTAL:'} price={totalPrice}/>
    </ul>
  )
}
